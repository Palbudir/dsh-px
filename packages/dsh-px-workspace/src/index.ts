...
import { readFailure } from '../../dsh-px-taskflow/src/index'

export const name = 'dsh-px-workspace'
export const inject: string[] = []
export const DEFAULTS = { routePrefix: '/dsh-px-workspace' }
interface Session { header: { id: string, cwd?: string, origin?: string }, snapshotEvents: () => readonly RecordEvent[] }
interface Host {
  sessions: { get: (id: string) => Session | undefined }
  sessionController: {
    inspect: (id: string) => Promise<{ meta: Session['header'], events: readonly RecordEvent[] }>
    prompt: (request: { requestId: string, sessionId: string, mode: 'queue', content: Array<{ type: 'text', text: string }>, clientTimeZone: string }, signal: AbortSignal) => Promise<{ accepted: true }>
  }
...
async function body (req: IncomingMessage): Promise<any> {
  ...
  try {
    ...
    return data
  } catch { throw new InputError('JSON 内容无效') }
}
...
export function apply (ctx: { inject: (services: string[], cb: (host: Host) => void) => unknown }): void {
  ctx.inject(['webServer', 'sessions', 'sessionController'], host => {
    ...
    let store: WorkspaceStore | undefined, scheduler: Scheduler | undefined, loadError = ''
    const inspect = async (id: string): Promise<{ meta: Session['header'], events: readonly RecordEvent[] }> => {
    ...
    try {
      ...
      scheduler = new Scheduler(store, async (schedule, requestId) => {
        ...
        const result = await host.sessionController.prompt({ requestId, sessionId: schedule.sessionId, mode: 'queue', clientTimeZone: schedule.timeZone,
          content: [{ type: 'text', text: `定时任务「${schedule.title}」\n这是用户保存的定时任务，请按当前会话的权限执行：\n\n${schedule.prompt}` }] }, lifetime.signal)
        if (result?.accepted !== true) throw new Error('会话未确认接收')
      })
    } catch (error) { loadError = String(error); host.logger?.warn(name, loadError) }
    host.effect(() => {
      const timer = setInterval(() => { void scheduler?.tick().catch(error => host.logger?.warn('定时任务检查失败', String(error))) }, 2000)
      timer.unref()
      return () => { clearInterval(timer); scheduler?.stop(); lifetime.abort() }
    }, 'workspace: scheduler')
    for (const route of ['content', 'message', 'annotations', 'schedules'] as const) host.effect(() => host.webServer.register({ kind: 'exact', path: `/${name}/${route}`, handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST') {
          ...
          if (route === 'annotations') {
            if (input.action === 'delete') { store.deleteAnnotation(input); return send(200, { ok: true }) }
          ...
          if (input.action === 'save') { await target(identifier(input.sessionId)); return send(200, scheduler.save(input)) }
          if (input.action === 'delete') { scheduler.remove(input); return send(200, { ok: true }) }
        ...
        if (route === 'message') {
          ...
          return send(200, { ...m, text: m.text.slice(offset, offset + 32000), length, offset, nextOffset: offset + 32000 < length ? offset + 32000 : null })
        }
