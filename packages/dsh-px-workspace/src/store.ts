...
import { nextOccurrence, type Annotation, type Message, type Schedule, type Timing, type WorkspaceState } from './model'
...
export function timing (value: any): Timing {
  if (value?.kind === 'once' && Number.isSafeInteger(value.at) && value.at > 0 && value.at < 8640000000000000) return { kind: 'once', at: value.at }
...
function finite (n: unknown): boolean { return typeof n === 'number' && Number.isFinite(n) && n >= 0 }
function validateState (v: any): asserts v is WorkspaceState {
  if (v?.version !== 1 || !Array.isArray(v.annotations) || !Array.isArray(v.schedules) || v.annotations.length > 2000 || v.schedules.length > 100) throw new Error('工作区数据格式不受支持')
  const ids = new Set<string>()
  for (const a of v.annotations) {
    identifier(a.id); identifier(a.sessionId); identifier(a.messageId)
    text(a.quote, 8000); text(a.note, 4000, false)
  ...
  for (const s of v.schedules) {
    identifier(s.id); identifier(s.sessionId); text(s.title, 100); text(s.prompt, 4000); timing(s.timing)
    ...
    for (const h of s.history) if (!finite(h.time) || !['dispatching', 'queued', 'uncertain'].includes(h.status) || typeof h.requestId !== 'string' || (h.detail !== undefined && typeof h.detail !== 'string')) throw new Error('投递记录损坏')
    ids.add(s.id)
  }
}
export class WorkspaceStore {
  ...
  constructor (readonly path: string) {
    try {
      ...
      validateState(parsed); this.state = parsed
    } catch (error: any) {
  ...
  update (fn: (state: WorkspaceState) => void): void {
    const next = this.snapshot(); fn(next); validateState(next)
  ...
  saveAnnotation (input: any, source: Message, now = Date.now()): Annotation {
    ...
    const quote = text(input.quote, 8000), note = text(input.note, 4000, false)
...
export interface ScheduleInput { id?: string, updatedAt?: number, title: string, sessionId: string, prompt: string, timing: Timing, enabled: boolean }
export class Scheduler {
  private busy = new Set<string>()
  private stopped = false
  constructor (readonly store: WorkspaceStore, private readonly deliver: (schedule: Schedule, requestId: string) => Promise<void>, readonly zone = Intl.DateTimeFormat().resolvedOptions().timeZone, private readonly now = Date.now) {
    ...
    if (interrupted) store.update(state => {
      for (const s of state.schedules) if (s.history.some(h => h.status === 'dispatching')) {
        s.enabled = false; s.nextAt = null; s.updatedAt = this.now()
        for (const h of s.history) if (h.status === 'dispatching') { h.status = 'uncertain'; h.detail = '上次投递期间服务中断；请先检查目标会话，确认后再启用或立即投递。' }
      }
    })
  }
  ...
  save (input: ScheduleInput): Schedule {
    const id = input.id === undefined ? randomUUID() : identifier(input.id)
    this.guard(id)
    const title = text(input.title, 100), prompt = text(input.prompt, 4000), sessionId = identifier(input.sessionId), rule = timing(input.timing)
  ...
  async tick (): Promise<void> {
    ...
    for (const s of due) {
      ...
      if (s.timeZone !== this.zone) {
        this.store.update(state => { const row = state.schedules.find(r => r.id === s.id)!; row.enabled = false; row.nextAt = null; row.updatedAt = this.now(); row.history = [{ requestId: randomUUID(), time: this.now(), status: 'uncertain' as const, detail: '本机时区已变化，任务暂停；请编辑确认时间后启用。' }, ...row.history].slice(0, 20) })
  ...
  async run (id: string): Promise<Schedule> {
    ...
    try {
      this.store.update(state => {
        ...
        if (s.timing.kind === 'once') { s.enabled = false; s.nextAt = null }
        else if (s.enabled && (s.nextAt ?? 0) <= now) s.nextAt = nextOccurrence(s.timing, now)
      })
      let detail: string | undefined
      try { await this.deliver(schedule, requestId) } catch (error) { detail = '投递未确认，请先检查目标会话再重试。' + String(error).slice(0, 500) }
      ...
      try { this.store.update(state => {
        const s = state.schedules.find(s => s.id === id)!
        const h = s.history.find(h => h.requestId === requestId)!
        h.status = detail ? 'uncertain' : 'queued'; h.detail = detail
        s.updatedAt = Math.max(this.now(), s.updatedAt + 1)
        if (detail) { s.enabled = false; s.nextAt = null }
      }) } catch { this.stop(); throw new InputError('投递结果无法保存；调度已停止。请检查目标会话与数据目录后重启应用。', 503) }
