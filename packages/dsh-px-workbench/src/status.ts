...
export function readServiceState (userData = process.env.DSH_PX_USER_DATA): ServiceState | null {
  ...
  try {
    ...
    if (!['starting', 'running', 'restarting', 'error', 'stopped'].includes(value.phase) ||
        typeof value.message !== 'string' || !Number.isFinite(Date.parse(value.updatedAt))) return null
    // 心跳过期不能继续显示为“运行正常”。
...
export function localStatus (): LocalStatus {
  ...
  let writable = false
  if (home) {
    try { accessSync(home, constants.W_OK); writable = true } catch { /* reported */ }
  }
  try {
    ...
    for (const [name, requested] of Object.entries(pkg.dependencies ?? {})) {
      ...
      try { version = JSON.parse(readFileSync(join(profile, 'node_modules', name, 'package.json'), 'utf8')).version ?? null } catch { /* missing */ }
      plugins.push({ name, requested: String(requested), version, enabled: pkg.dsh?.profile?.bundles?.includes(name) === true })
    }
