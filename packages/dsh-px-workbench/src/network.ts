export interface FetchResult { statusCode: number, body: { content: string }, truncated: boolean }
export type FetchPage = (request: { url: string }, signal?: AbortSignal) => Promise<FetchResult>
export interface NetworkCheck {
  checkedAt: string
  checks: { url: string, ok: boolean, status: number | null, chars: number, truncated: boolean, message: string }[]
}
...
function explanation (error: unknown): string {
  ...
  if (code === 'WEB_ABORTED' || code === 'WEB_FETCH_TIMEOUT' || (error as Error)?.name === 'TimeoutError') return '读取超时，请检查本机代理是否运行。'
...
export async function checkWebAccess (fetchPage: FetchPage): Promise<NetworkCheck> {
  return { checkedAt: new Date().toISOString(), checks: await Promise.all(targets.map(async url => {
    try {
      ...
      return { url, ok, status: result.statusCode, chars: result.body.content.length, truncated: result.truncated,
