...
export function apply (ctx: HostPluginContext): void {
  ctx.inject(['webServer', 'workspaceController'], (ctx) => {
    ...
    const dispose = [
      ctx.webServer.register({ kind: 'exact', path: `${DEFAULTS.routePrefix}/status`, handler: (req, res) => {
        if (req.method !== 'GET') return json(res, 405, { error: '请使用 GET' })
        json(res, 200, localStatus())
      } }),
      ctx.webServer.register({ kind: 'exact', path: `${DEFAULTS.routePrefix}/restart`, handler: (req, res) => {
        ...
        } catch { json(res, 503, { error: '无法提交重启请求，请通过桌面托盘重试。' }) }
      } }),
      ctx.webServer.register({ kind: 'exact', path: `${DEFAULTS.routePrefix}/workspace`, handler: async (req, res) => {
        if (req.method !== 'POST') return json(res, 405, { error: '请使用 POST' })
        if (req.headers?.['x-dsh-px-request'] !== '1') return json(res, 403, { error: '请从本机工作台提交请求' })
        ...
        const controller = (ctx as HostPluginContext & {
        ...
        } catch (err) { json(res, 400, { error: `无法添加工作区，请确认文件夹存在并可访问：${err instanceof Error ? err.message : String(err)}` }) }
      } })
    ]
