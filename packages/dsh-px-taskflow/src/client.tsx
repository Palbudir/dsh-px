      ...
      {data ? (
        <>
          <section style={card} aria-label="任务记录">
            {data.checkpoint ? (
              <>
                ...
                <small style={{ opacity: 0.65 }}>
                  Agent 工作记录 · {new Date(data.checkpoint.time).toLocaleString()}
                </small>
                ...
                {data.checkpoint.evidence.length ? (
                  <details>
                    ...
                    {data.checkpoint.evidence.map((id) => {
                      ...
                      return call ? (
                        <ExecutionCard key={id} call={call} sessionId={scope.sessionId} />
                      ) : (
                        <div key={id}>
          ...
          {data.changedFiles.length ? (
            <section style={card}>
              <strong>已记录的文件写入 · {data.changedFiles.length} 个路径</strong>
              ...
              <p style={{ marginBottom: 0, opacity: 0.65 }}>
                来自成功返回的文件写工具，不包含只读查看。命令行改动、最终差异与人工已有修改请在文件变动面板核对。
              </p>
            </section>
          ) : null}
          <section aria-label="实际执行记录">
            ...
            {[...data.executions].reverse().map((call) => (
              <ExecutionCard key={call.id} call={call} sessionId={scope.sessionId} />
            ))}
