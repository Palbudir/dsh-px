import { evidenceDetail, integerOption, reviewEvents, validateCheckpoint, type Event, type ReviewOptions, type EvidenceOptions } from './evidence'
...
export function readFailure (error: unknown): { status: number, code: string, error: string, retryable: boolean } {
...
function queryInteger (params: URLSearchParams, key: string, fallback: number, min: number, max: number): number {
...
export function apply (ctx: Context): void {
  ...
  ctx.inject(['tools'], host => {
    ...
    host.tools.register({ name: 'task_review', description: '读取当前任务工作记录与执行摘要，默认最近 20 条。用 nextBeforeSeq 作为 beforeSeq 翻阅历史；需输出正文时用 task_evidence。returned 不代表测试通过。',
      parameters: { type: 'object', properties: { beforeSeq: { type: 'integer', minimum: 0 }, limit: { type: 'integer', minimum: 1, maximum: 50 } }, additionalProperties: false }, output,
      execute: async (args: ReviewOptions, exec: Run) => JSON.stringify(reviewEvents(session(exec).snapshotEvents(), true, args)) })
    host.tools.register({ name: 'task_evidence', description: '按真实 callId 读取当前会话的一条执行及其持久输出，支持历史记录。默认 4000 字符，用 nextOutputOffset 继续读取。只读取日志，不重跑命令；上游已截断或未保存的内容无法恢复。',
      parameters: { type: 'object', required: ['callId'], properties: { callId: { type: 'string', minLength: 1, maxLength: 200 }, offset: { type: 'integer', minimum: 0 }, maxChars: { type: 'integer', minimum: 1, maximum: 8000 } }, additionalProperties: false }, output,
  ...
  ctx.inject(['webServer', 'sessions', 'sessionPersistence'], host => {
    for (const kind of ['review', 'evidence'] as const) host.effect(() => host.webServer.register({ kind: 'exact', path: `${DEFAULTS.routePrefix}/${kind}`, handler: async (req: any, res: any) => {
      ...
      const invalid = (error: string): void => send(400, { error, code: 'INVALID_REQUEST', retryable: false })
      ...
      try {
        ...
        if (kind === 'review') {
          ...
          const options = { offset: queryInteger(params, 'offset', 0, 0, Number.MAX_SAFE_INTEGER), maxChars: queryInteger(params, 'maxChars', 4000, 1, 8000) }
          select = (events, live) => evidenceDetail(events, callId, live, options)
        }
      } catch { return invalid('分页参数无效') }
      ...
      try {
        ...
        try { ({ events } = await handle.read()) } catch (error) { readFailed = true; throw error } finally {
          try { await handle.close() } catch (closeError) { if (!readFailed) throw closeError }
        }
        respond(events, false)
      } catch (error) { const failure = readFailure(error); send(failure.status, failure) }
    } }), `taskflow: ${kind} route`)
  })
}
