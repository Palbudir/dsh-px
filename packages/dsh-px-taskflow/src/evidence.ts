...
export interface Execution {
  ...
  effect: 'read' | 'write' | 'unknown', outcome: Outcome,
  outcomeSource: 'tool' | 'command_marker' | 'turn' | 'pending',
  output: string, outputLength: number, outputTruncated: boolean,
  time: number, durationMs: number | null
}
export interface ReviewOptions { beforeSeq?: number, limit?: number }
export interface EvidenceOptions { offset?: number, maxChars?: number }
...
const reads = new Set(['read', 'ls', 'list', 'glob', 'grep', 'search', 'web_search', 'web_fetch', 'present', 'job_output', 'job_list', 'task_review', 'task_evidence'])
...
function resultStatus (tool: string, text: string, isError: boolean): Pick<Execution, 'outcome' | 'outcomeSource'> {
...
interface Folded { calls: Map<string, Execution>, relevant: Execution[], checkpoint: TaskReview['checkpoint'] }
function foldEvents (events: readonly Event[], live: boolean): Folded {
  ...
  for (const event of events) {
    ...
    if (['turn/start', 'turn/end', 'session/end-seed'].includes(event.type)) {
      ...
      for (const call of pending) { call.outcome = cancelled ? 'cancelled' : 'interrupted'; call.outcomeSource = 'turn' }
      pending.clear()
    ...
    if (event.type === 'tool/call' || event.type === 'tool/ptc-dispatch-start') {
      ...
      const call: Execution = { id, seq: event.seq, tool: data.name,
        ...
        effect, outcome: 'running', outcomeSource: 'pending', output: '', outputLength: 0, outputTruncated: false,
        time: event.time, durationMs: null }
      calls.set(id, call); pending.add(call)
    }
    if (event.type === 'tool/result' || event.type === 'tool/ptc-dispatch') {
      const blocks = event.type === 'tool/result' ? (data.message?.content ?? []).filter((b: any) => b.type === 'tool-result') : [data]
      for (const block of blocks) {
        ...
        pending.delete(call)
        call.output = text; call.outputLength = text.length
  ...
  if (!activeTurn || !live) for (const call of pending) { call.outcome = 'interrupted'; call.outcomeSource = 'turn' }
...
export function evidenceDetail (events: readonly Event[], id: string, live = true, options: EvidenceOptions = {}): EvidenceDetail | null {
  ...
  return { ...call, output: call.output.slice(offset, offset + size), outputOffset: offset,
    outputTruncated: offset > 0 || next !== null, nextOutputOffset: next }
}
